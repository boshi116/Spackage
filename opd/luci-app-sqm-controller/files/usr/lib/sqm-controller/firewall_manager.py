...
负责把分类规则翻译成 nftables 或 iptables 规则，并维护分类链的完整性。
这里关注的是“命中什么条件就打什么 mark”，不处理策略调度本身。
"""
import argparse
import json
import os
...
def run_cmd_input(cmd, input_text, timeout=DEFAULT_CMD_TIMEOUT):
    try:
...
def run_checked(cmd, details, ok_rc=(0,), timeout=DEFAULT_CMD_TIMEOUT):
    result = run_cmd(cmd, timeout=timeout)
    details["commands"].append(result)
    ok = result["rc"] in ok_rc
    if result["rc"] == -1:
        ok = False
    return ok, result


def run_checked_input(cmd, input_text, details, ok_rc=(0,), timeout=DEFAULT_CMD_TIMEOUT):
    result = run_cmd_input(cmd, input_text, timeout=timeout)
    details["commands"].append(result)
    ok = result["rc"] in ok_rc
    if result["rc"] == -1:
        ok = False
...
def normalize_rules(raw_rules, category_marks):
    # 统一把前端/配置里的规则整理成标准结构，后续生成 nft/iptables 时只认这一层。
    ...
    for idx, item in enumerate(raw_rules, start=1):
        ...
        dst_ip_match = str(item.get("dst_ip", "")).strip()
...
def build_nft_match_tokens(rule, dport_token, sport_token):
    ...
    if rule.get("dst_ip"):
        if ":" in rule["dst_ip"]:
            tokens.extend(["ip6", "daddr", rule["dst_ip"]])
        else:
            tokens.extend(["ip", "daddr", rule["dst_ip"]])
    if dport_token:
...
def _nft_join(tokens):
    return " ".join(str(item) for item in tokens if item is not None and str(item) != "")
...
def apply_nft(rules, nft_path):
    ...
    batch_lines = [
        f"flush chain {NFT_TABLE_FAMILY} {NFT_TABLE_NAME} {NFT_CHAIN_NAME}",
        f"add rule {NFT_TABLE_FAMILY} {NFT_TABLE_NAME} {NFT_CHAIN_NAME} ct mark != 0x0 meta mark set ct mark",
    ]
    for rule in rules:
        ...
        for dport_token in dport_tokens:
            for sport_token in sport_tokens:
                tokens = [
                    "add", "rule", NFT_TABLE_FAMILY, NFT_TABLE_NAME, NFT_CHAIN_NAME,
                    *build_nft_match_tokens(rule, dport_token, sport_token),
                    "counter", "meta", "mark", "set", rule["mark_hex"], "ct", "mark", "set", "meta", "mark",
                ]
                batch_lines.append(_nft_join(tokens))
                details["rules_applied"] += 1

    batch = "\n".join(batch_lines) + "\n"
    ok, _ = run_checked_input([nft_path, "-f", "-"], batch, details)
    if not ok:
        return False, "failed to atomically replace nft chain rules", details
...
def apply_iptables(rules, iptables_path):
    details = {"commands": [], "rules_in": len(rules), "rules_applied": 0}
    tmp_chain = f"{IPT_CHAIN}_NEW"
    old_chain = f"{IPT_CHAIN}_OLD"
    ...
    run_checked([iptables_path, "-t", IPT_TABLE, "-F", tmp_chain], details, ok_rc=(0, 1))
    run_checked([iptables_path, "-t", IPT_TABLE, "-X", tmp_chain], details, ok_rc=(0, 1))
    run_checked([iptables_path, "-t", IPT_TABLE, "-F", old_chain], details, ok_rc=(0, 1))
    run_checked([iptables_path, "-t", IPT_TABLE, "-X", old_chain], details, ok_rc=(0, 1))
    ok, _ = run_checked([iptables_path, "-t", IPT_TABLE, "-N", tmp_chain], details)
    if not ok:
        return False, "failed to create temporary iptables chain", details
    ...
    for rule in rules:
        ...
        for dport_token in dport_tokens:
            for sport_token in sport_tokens:
                base_matches = []
                if rule["proto"] != "all":
                    base_matches.extend(["-p", rule["proto"]])
                if rule["ip"]:
                    base_matches.extend(["-s", rule["ip"]])
                if rule.get("dst_ip"):
                    base_matches.extend(["-d", rule["dst_ip"]])
                if dport_token:
                    base_matches.extend(["--dport", dport_token.replace("-", ":")])
                if sport_token:
                    base_matches.extend(["--sport", sport_token.replace("-", ":")])

                mark_matches = ["-m", "mark", "--mark", "0x0/0xffffffff", *base_matches]
                save_matches = ["-m", "mark", "--mark", f"{rule['mark_hex']}/0xffffffff", *base_matches]
    ...
    ok, _ = run_checked([iptables_path, "-t", IPT_TABLE, "-E", IPT_CHAIN, old_chain], details)
    if not ok:
        return False, "failed to move old iptables chain aside", details
    ok, _ = run_checked([iptables_path, "-t", IPT_TABLE, "-E", tmp_chain, IPT_CHAIN], details)
    if not ok:
        run_checked([iptables_path, "-t", IPT_TABLE, "-E", old_chain, IPT_CHAIN], details, ok_rc=(0, 1))
        return False, "failed to activate new iptables chain", details
    ok, check_current = run_checked(
        [iptables_path, "-t", IPT_TABLE, "-C", "PREROUTING", "-j", IPT_CHAIN],
        details,
        ok_rc=(0, 1),
    )
    if not ok:
        return False, "failed to verify iptables jump after chain swap", details
    if check_current["rc"] == 1:
        ok, _ = run_checked([iptables_path, "-t", IPT_TABLE, "-I", "PREROUTING", "-j", IPT_CHAIN], details)
        if not ok:
            return False, "failed to add iptables jump after chain swap", details
    for _ in range(32):
        ok, check_old = run_checked(
            [iptables_path, "-t", IPT_TABLE, "-C", "PREROUTING", "-j", old_chain],
            details,
            ok_rc=(0, 1),
        )
        if not ok or check_old["rc"] == 1:
            break
        run_checked([iptables_path, "-t", IPT_TABLE, "-D", "PREROUTING", "-j", old_chain], details, ok_rc=(0, 1))
    run_checked([iptables_path, "-t", IPT_TABLE, "-F", old_chain], details, ok_rc=(0, 1))
    run_checked([iptables_path, "-t", IPT_TABLE, "-X", old_chain], details, ok_rc=(0, 1))

    details["rules"] = rules
    details["transactional"] = "temp-chain-rename"
    return True, "", details
