...
VALID_POLICY_MODES = {"auto", "balanced", "gaming", "live", "meeting", "download", "streaming", "bulk"}
POLICY_MODE_INTERNAL = {
    "auto": "auto",
    "balanced": "balanced",
    "gaming": "gaming",
    "live": "live",
...
class UCISectionManager:
    ...
    def save(self):
        """原子写入：tmp 文件 → fsync → os.replace。失败时保留原文件。"""
...
class ConfigManager:
    ...
    def _refresh_views(self):
        ...
        self.adaptive = dict((self.section_manager.get_first("policy", "adaptive") or {}).get("options", {}))
        classifier_section = (
            self.section_manager.get_first("classifier", "enhanced")
            or self.section_manager.get_first("classifier", "v" + "4")
            or {}
        )
        self.enhanced_classifier = dict(classifier_section.get("options", {}))
        self.class_rules = [
    ...
    def load_config(self):
        """加载 UCI 配置。失败时保留旧配置，不覆盖内存中的 sections。"""
        old_sections = list(self.section_manager.sections)
        old_basic = dict(self.basic_config)
        try:
            self.section_manager.config_path = self.config_path
            self.section_manager.load()
        except Exception as exc:
            self.logger.error("Failed to parse config: %s", exc)
            return self._restore_on_load_failure(old_sections, old_basic, str(exc))

        # 解析成功但内容为空（损坏或空文件）→ 视为加载失败
        if not self.section_manager.sections:
            self.logger.error("Config parsed but contained no sections")
            return self._restore_on_load_failure(old_sections, old_basic,
                                                  "config file is empty or contains no valid sections")

        # 必须包含 basic_config section
        has_basic = any(s.get("type") == "basic_config" for s in self.section_manager.sections)
        if not has_basic:
            self.logger.error("Config missing required 'basic_config' section")
            return self._restore_on_load_failure(old_sections, old_basic,
                                                  "config missing required basic_config section")

        self._refresh_views()
        self.logger.info(
            "Config loaded: basic=%d advanced=%d classification=%d policy=%d class_rules=%d",
            len(self.basic_config), len(self.advanced_config),
            len(self.classification), len(self.policy), len(self.class_rules),
        )
        return self.config

    def _restore_on_load_failure(self, old_sections, old_basic, reason):
        """加载失败时回退到旧配置，不清空内存视图。"""
        self.section_manager.sections = old_sections
        if old_basic:
            self._refresh_views()
            self.logger.warning(
                "Config load failed (%s), using previously loaded config (basic=%d rules=%d)",
                reason, len(self.basic_config), len(self.class_rules),
            )
        else:
            self.basic_config = {}
            self.advanced_config = {}
            ...
            self.enhanced_classifier = {}
            self.class_rules = []
            self.logger.warning("Config load failed (%s), no previous config available", reason)
        return {"error": "config_load_failed", "detail": reason}
    ...
    def get_settings(self):
        ...
        return {
            ...
            "enhanced_classifier": self.enhanced_classifier.copy(),
    ...
    def get_adaptive_settings(self):
        if not self.section_manager.sections and os.path.exists(self.config_path):
            self.load_config()
        settings = self.adaptive.copy() if self.adaptive else {}
        if not settings:
            settings = self.policy.copy() if self.policy else {}
        product_mode = str(settings.get("mode", "auto")).strip().lower()
        if product_mode not in VALID_POLICY_MODES:
            product_mode = "auto"
        mode = POLICY_MODE_INTERNAL.get(product_mode, "auto")
        bulk_cap_pct = int(settings.get("bulk_cap_pct", settings.get("bulk_ceil_pct", 60)))
        return {
            "enabled": str(settings.get("enabled", "1")).strip().lower() in {"1", "true", "yes", "on"},
            "mode": mode,
            "product_mode": product_mode,
            "mode_label": POLICY_MODE_LABELS.get(product_mode, product_mode),
            "latency_high_ms": float(settings.get("latency_high_ms", 80)),
            "loss_high_pct": float(settings.get("loss_high_pct", 2)),
            "jitter_high_ms": float(settings.get("jitter_high_ms", 30)),
            "cooldown_min": int(settings.get("cooldown_min", 2)),
            "debounce_count": int(settings.get("debounce_count", 2)),
            "gaming_floor_pct": int(settings.get("gaming_floor_pct", 15)),
            "streaming_floor_pct": int(settings.get("streaming_floor_pct", 25)),
            "bulk_cap_pct": bulk_cap_pct,
            "bulk_ceil_pct": bulk_cap_pct,
            "unknown_high_pct": float(settings.get("unknown_high_pct", 30)),
        }

    def get_classifier_enhancement_settings(self):
        if not self.section_manager.sections and os.path.exists(self.config_path):
            self.load_config()
        settings = self.enhanced_classifier.copy() if self.enhanced_classifier else {}
        return {
            "unknown_log_enabled": str(settings.get("unknown_log_enabled", "1")).strip().lower() in {"1", "true", "yes", "on"},
            "confidence_threshold": float(settings.get("confidence_threshold", 0.65)),
            "user_rule_enabled": str(settings.get("user_rule_enabled", "1")).strip().lower() in {"1", "true", "yes", "on"},
        }
    ...
    def set_value(self, key, value, section=None):
        ...
        if target_section is None:
            # 自动判断：仅在已知字段白名单内生效
            if key in self._BASIC_CONFIG_KEYS:
                target_section = "basic_config"
            elif key in self._ADVANCED_CONFIG_KEYS:
                target_section = "advanced_config"
            # 也在当前已加载的配置中查找（兼容用户自定义字段）
            elif key in self.basic_config:
                target_section = "basic_config"
            elif key in self.advanced_config:
                target_section = "advanced_config"
            else:
                self.logger.error(
                    "set_value: unknown key '%s' without explicit section, refusing to guess",
                    key,
                )
                return False
    ...
    def save_config(self):
        # 防止空 sections 覆盖原配置文件
        if not self.section_manager.sections:
            self.logger.error("Refusing to save empty sections (config may not be loaded)")
            return False
        has_basic = any(s.get("type") == "basic_config" for s in self.section_manager.sections)
        if not has_basic:
            self.logger.error("Refusing to save config missing basic_config section")
            return False
        try:
            self.section_manager.config_path = self.config_path
    ...
    for idx, left in enumerate(prepared):
        for right in prepared[idx + 1:]:
            ...
            if not _src_ip_overlap(left["src_ip"], right["src_ip"]):
                continue
            if not _src_ip_overlap(left["dst_ip"], right["dst_ip"]):
                continue
...
def validate_config_file(path):
    ...
    for item in class_rules:
        ...
        dst_ip = str(options.get("dst_ip", "")).strip()
        if dst_ip and _parse_ip_value(dst_ip) is None:
            result["errors"].append(f"{name}: dst_ip '{dst_ip}' must be an IP address or CIDR")

    if policy:
        mode = str(policy.get("mode", "auto")).strip().lower() or "auto"
        if mode not in VALID_POLICY_MODES:
            result["errors"].append("policy.mode must be one of auto/balanced/gaming/live/meeting/download")
        ...
        def _policy_int(field_name, default=0):
            try:
                return int(str(policy.get(field_name, default) or default).strip())
            except Exception:
                return default

        if (
            _policy_int("gaming_floor_pct") +
            _policy_int("streaming_floor_pct")
        ) > 92:
            result["errors"].append("policy gaming_floor_pct + streaming_floor_pct must be <= 92")

        cron_expr = (
            str(policy.get("cron_expression", "")).strip()
            or str(policy.get("cron", "")).strip()
            or str(policy.get("schedule", "")).strip()
        )
