...
def _state_path(dev, state_key=None):
    suffix = ""
    if state_key:
        suffix = "_" + _safe_dev_name(state_key)
    return f"{STATE_PREFIX}{_safe_dev_name(dev)}{suffix}.json"
...
def collect(dev, state_key=None, update_state=True):
    ...
    state_path = _state_path(dev, state_key=state_key)
...
def main():
    ...
    parser.add_argument("--state-key", default="", help="独立速率采样状态名，例如 policy/ui")
    parser.add_argument("--no-update-state", action="store_true", help="只读当前计数，不更新采样状态")
